...
const Pricing = ({pricingData}) => {
    ...
    return (
        <section className='my-5' style={{display: pricingData?.isShow ? 'block': 'none' }}>
           <Card className={`${pricingStyles.priceCard} mx-auto  p-3 border-0`}>
                <Card.Body>
                    <Card.Title className='fs-2 fw-bold pt-3'>{pricingData?.title}</Card.Title>
                    <Card.Subtitle className='fw-normal my-3'>{pricingData?.description}</Card.Subtitle>
                    <div>
                    <Row className="g-4">
                        {
