...
const ContactForm = ({contactUsForm}) => {
  ...
  const handleFormSubmit = e => {
    ...
    return (
        <Card className={`${contactFormStyles.contactForm} mx-auto p-2 p-lg-5 shadow-md bg-white rounded border-0`} style={{display: contactUsForm?.isShow ? 'block' : 'none'}}>
          <Card.Body>
            <Form onSubmit={handleFormSubmit}>
              <h1 className='fs-1 fw-bold pb-4'>{contactUsForm?.title}</h1>
